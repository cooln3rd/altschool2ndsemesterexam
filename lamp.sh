...
sudo mysql -uroot -e "CREATE USER 'ebuben'@'localhost' IDENTIFIED BY 'password';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON laraveldb.* TO 'ebuben'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '26 s/=root/=ebuben/' /var/www/laravel/.env
sudo sed -i '27 s/=/= password/' /var/www/laravel/.env
